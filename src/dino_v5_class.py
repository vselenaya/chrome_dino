...
class chrome_dino:
    def __init__(self, based):
        ...
        """
            при инициации класса chrome_dino будет делаться снимок экрана, на котором узнаются параметры экрана,
            с коротыми опредляются почти все остальные переменные: цвет фона, цвет динозавра и препятствий, 
            размеры снимка экрана для цикла, области обзора, высота основания, коэффициент изменения области обзора и
            прочие константы, которые нужно посчитать только один раз, чтобы не замедлять программу
        """
    ...
    def view_area(self):
        """
            области обзора меняются линейно от коэффициента времени, что был определен раньше
        """
...
"""
    перед инициацией chrome_dino необходимо найти высоту основания, которую мы и подадим на вход
"""
